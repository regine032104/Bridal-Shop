    ...
    <div class="flex flex-col md:flex-row gap-8">
        ...
        <aside
            ...
            <div class="flex flex-col items-center md:items-start w-full">
                <div
                    class="w-20 h-20 rounded-full bg-neutral-200 mb-2 overflow-hidden flex items-center justify-center">
                    <!-- Avatar: use user's image if available, else fallback -->
                    <?php
                    $imgPath = '../img/' . ($userData['profile_img'] ?? '');
                    if (!empty($userData['profile_img']) && file_exists($imgPath)) {
                        // Add cache-busting query string
                        $imgSrc = $imgPath . '?v=' . time();
                    } else {
                        $imgSrc = '../img/default-avatar.png';
                    }
                    ?>
                    <img src="<?= $imgSrc ?>" alt="Profile" class="w-full h-full object-cover rounded-full" />
                </div>
            ...
            <nav class="w-full">
                <ul class="space-y-1 text-base">
                    ...
                    <li><a href="#" id="sidebar-cart"
                            class="sidebar-link flex items-center gap-2 rounded-lg px-4 py-2">My Cart</a></li>
                    <li><a href="#" id="sidebar-orders"
                            class="sidebar-link flex items-center gap-2 rounded-lg px-4 py-2">My Orders</a></li>
            ...
            <div id="section-profile">
                <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
                    <form method="post" enctype="multipart/form-data"
                        class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start" id="profile-info-form">
                        <div class="space-y-6">
                            <div>
                                <label class="block text-sm text-slate-700 mb-1">Username</label>
                                <input type="text" class="form-input w-full"
                                    value="<?= htmlspecialchars($userData['username'] ?? $user_name); ?>"
                                    name="username" readonly />
                                <span class="text-xs text-slate-500">Username can only be changed once.</span>
                            </div>
                            <script>
                                // Activate Edit Profile link to scroll to the profile form
                                document.addEventListener('DOMContentLoaded', function () {
                                    ...
                                    if (editLink) {
                                        editLink.addEventListener('click', function (e) {
                                            ...
                                            document.querySelector('form input[name="first_name"]').focus();
                                        });
                                    }
                                });
                            </script>
                            <div>
                                ...
                                <input type="text" class="form-input w-full"
                                    value="<?= htmlspecialchars($userData['first_name']); ?>" name="first_name" />
                            </div>
                            <div>
                                <label class="block text-sm text-slate-700 mb-1">Email</label>
                                <input type="email" class="form-input w-full"
                                    value="<?= htmlspecialchars($userData['email']); ?>" name="email" />
                            </div>
                            <div>
                            ...
                            <div>
                                <label class="block text-sm text-slate-700 mb-1">Gender <span
                                        class="text-xs text-slate-400" title="Gender is required">?</span></label>
                            ...
                            <div>
                                <label class="block text-sm text-slate-700 mb-1">Date of birth <span
                        ...
                        <div class="flex flex-col items-center justify-center gap-4">
                            <div
                                class="w-32 h-32 rounded-full bg-neutral-200 overflow-hidden flex items-center justify-center mb-2">
                                <?php
                                $imgPath = '../img/' . ($userData['profile_img'] ?? '');
                                if (!empty($userData['profile_img']) && file_exists($imgPath)) {
                                    $imgSrc = $imgPath . '?v=' . time();
                                    ...
                                    <?php foreach ($products as $product): ?>
                                        <tr class="border-t border-pink-100">
                                            ...
                                            <td class="p-4">
                                                ...
                                                <small
                                                    class="text-pink-600"><?= htmlspecialchars($product['material']) ?></small>
                                            </td>
                                ...
                                <div class="border rounded-xl p-4 mb-2 bg-gray-50">
                                        ...
                                        <div class="mb-2 space-y-2">
                                            <?php foreach ($orderItems[$order['order_id']] as $item): ?>
                                                <div class="flex items-center gap-3 rounded border bg-white p-2">
                                                    ...
                                                    <div class="flex-1">
                                                        <span
                                                            class="font-semibold text-slate-900"><?= htmlspecialchars($item['product_name']); ?></span>
                                    ...
                                    <div
                                        ...
                                        <div class="flex flex-col gap-2 sm:flex-row">
                                            <a href="order-detail.php?id=<?= (int) $order['order_id']; ?>"
                                                class="btn-secondary">View Details</a>
                                            <a href="../backend/order_receipt.php?id=<?= (int) $order['order_id']; ?>"
                                                class="btn-primary" target="_blank" rel="noopener">Download Receipt</a>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
